      ...
      signals.push({
        ...
        data: {
          ...
          window_seconds: Math.round(
            (new Date(current.created_at).getTime() - new Date(oldest.created_at).getTime()) / 1000,
          ),
...
async function poll(): Promise<{ prices: number; signals: number; ticks: number; errors: string[] }> {
  ...
  try {
    ...
    if (remaining !== null) {
      console.log(
        `[CG] Status: ${res.status}, rate-limit-remaining: ${remaining}, coins returned: ${Object.keys(priceData).length}`,
      );
    }
  } catch (e) {
  ...
  if (allDetected.length > 0) {
    ...
    const existingKeys = new Set((recentSignals || []).map((s: any) => `${s.coin_id}::${s.signal_type}`));
  ...
  await db
    .from("price_ticks")
    .delete()
    .lt("created_at", new Date(Date.now() - TICK_RETENTION_MS).toISOString());

  // Step 6: Cleanup expired signals (buffer beyond SIGNAL_EXPIRY_MS for late consumers)
  await db
    .from("bot_signals")
    .delete()
    .lt("created_at", new Date(Date.now() - SIGNAL_CLEANUP_MS).toISOString());
...
Deno.serve({ port: 8000 }, async (req) => {
  ...
  if (url.pathname === "/poll") {
    ...
    console.log(
      `[POLL] ${result.prices} prices, ${result.signals} signals, ${result.ticks} ticks written in ${elapsed}ms`,
    );
